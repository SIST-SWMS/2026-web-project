				...
				<td>
					...
					<c:choose>
						...
						<c:when test="${order.ovo.delivery_status=='상품준비중'}">
							<span class="badge"
								style="background:#fff3cd;color:#664d03;border:1px solid #ffecb5;font-size:14px;">
								상품준비중
							</span>
						</c:when>
						...
						<c:when test="${order.ovo.delivery_status=='배송중'}">
							<span class="badge"
								style="background:#cfe2ff;color:#084298;border:1px solid #b6d4fe;font-size:14px;">
								배송중
							</span>
						</c:when>
						...
						<c:when test="${order.ovo.delivery_status=='배송완료'}">
							<span class="badge"
								style="background:#d1e7dd;color:#0f5132;border:1px solid #badbcc;font-size:14px;">
								배송완료
							</span>
						</c:when>
						...
						<c:when test="${order.ovo.delivery_status=='주문취소'}">
							<span class="badge"
								style="background:#f8d7da;color:#842029;border:1px solid #f5c2c7;font-size:14px;">
								주문취소
							</span>
						</c:when>
	...
	<div class="card-body">
		<table class="table table-borderless align-middle mb-0">
			...
			<tr>
				<th>배송주소</th>
				<td>
					${order.ovo.delivery_addr}
					${order.ovo.delivery_addr_detail}
				</td>
			</tr>
			<tr>
				<th>배송메시지</th>
...
<div class="mt-4 text-end">
	<a href="../mypage/orderList.do" class="btn btn-outline-dark">
		이전
	</a>
</div>