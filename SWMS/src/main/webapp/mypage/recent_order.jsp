					...
					<td>
						...
						<div class="d-flex align-items-center gap-3">
							...
							<div>
								...
								<a href="../mypage/orderListDetail.do?order_no=${vo.ovo.order_no}" class="text-decoration-none text-dark"> ${vo.gvo.goods_name} </a>
					...
					<td class="text-center">
						...
						<c:choose>
							...
							<c:when test="${vo.ovo.delivery_status=='상품준비중'}">
								<span class="badge" style="background: #fff3cd; color: #664d03; border: 1px solid #ffecb5; font-size: 12px;"> 상품준비중 </span>
							</c:when>

							<c:when test="${vo.ovo.delivery_status=='배송중'}">
								<span class="badge" style="background: #cfe2ff; color: #084298; border: 1px solid #b6d4fe; font-size: 12px;"> 배송중 </span>
							</c:when>

							<c:when test="${vo.ovo.delivery_status=='배송완료'}">
								<span class="badge" style="background: #d1e7dd; color: #0f5132; border: 1px solid #badbcc; font-size: 12px;"> 배송완료 </span>
							</c:when>

							<c:when test="${vo.ovo.delivery_status=='주문취소'}">
								<span class="badge" style="background: #f8d7da; color: #842029; border: 1px solid #f5c2c7; font-size: 12px;"> 주문취소 </span>
							</c:when>

							<c:otherwise>
								<span class="badge bg-secondary"> ${vo.ovo.delivery_status} </span>
							</c:otherwise>
