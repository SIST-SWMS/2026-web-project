...
<body>
	...
	<div class="mb-3">
		<span class="d-inline-flex">
			<c:forEach var="i" begin="1" end="5">
				<c:if test="${i <= vo.hit}">
					<svg width="24" height="24" viewBox="0 0 12 12" class="text-warning">
						<use xlink:href="#star-solid"></use></svg>
				</c:if>
				<c:if test="${i > vo.hit}">
					<svg width="24" height="24" viewBox="0 0 12 12" class="text-secondary">
						<use xlink:href="#star-outline"></use></svg>
				</c:if>
			</c:forEach>
		</span>
		<span class="text-body-secondary small ms-2">${vo.created_at}</span>
	</div>
	...
	<div class="d-flex gap-2 justify-content-end border-top pt-3">
		...
		<a href="#" class="btn btn-outline-danger px-4" onclick="if(confirm('삭제하시겠습니까?')){ location.href='../mypage/review_delete.do?no=${vo.review_no}'; } return false;">삭제</a>
