...
<body>
	...
	<div class="d-flex align-items-center gap-3 pb-4 border-bottom mb-4">
		...
		<div>
			<div class="fw-bold">${qna.brand}</div>
	...
	<div class="mb-3">
		<label class="form-label fw-bold">문의 유형 <span
			...
			<option ${qna.type == '상품 문의'   ? 'selected' : ''}>상품 문의</option>
			<option ${qna.type == '배송 문의'   ? 'selected' : ''}>배송 문의</option>
			<option ${qna.type == '재입고 문의' ? 'selected' : ''}>재입고 문의</option>
			<option ${qna.type == '기타 문의'   ? 'selected' : ''}>기타 문의</option>
		</select>
	</div>
	...
	<div class="mb-3">
		<label class="form-label fw-bold">문의 상품옵션</label> <select
			class="form-select form-select-lg" name="option">
	...
	<div class="mb-2">
		...
		<input type="text" class="form-control form-control-lg" name="title"
			maxlength="30" value="${qna.title}">
	</div>

	<!-- 비밀글 (기존 값 checked) -->
	<div class="form-check mb-3">
		<input class="form-check-input" type="checkbox" id="secret"
			name="secret" ${qna.secret == 'Y' ? 'checked' : ''}> <label
	...
	<div class="mb-4">
		...
		<textarea class="form-control" name="content" rows="6">${qna.content}</textarea>
	</div>
